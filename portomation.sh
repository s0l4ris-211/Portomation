...
execution_fun() {
	...
	case $choice in
        ...
        2) # rustscan
        echo -en "\e[0;32m[*] Do you want to run the default scan for rustscan? (y/n):~ \e[0m"
        read run_default_scan
        echo -en "\n"
        ...
        if [ "$run_default_scan" == "y" ]; then 
        	...
        	echo -en "\e[0;32m[+] Please wait few seconds to finish the scan...\n\e[0m"
        	rustscan --accessible -a $target_ip -r1-65535 -u 65535 -b 10000 > $output_file
        	echo -en "\e[1;33m[+] Scan finished!\e[0m 🙂\n"
            cat $output_file
        else
			read -p "[+] Enter target IP:~ " target_ip 
        	read -p "[+] Enter additional flags for rustscan (e.g., '-u, --ulimit', '-p, --ports' etc):~ " flags
        	output_file="scan_result.rustscan"
        	echo -en "\e[1;32m[+] Please wait few seconds to finish the scan...\n\e[0m"
        	rustscan -a $target_ip $flags  > $output_file	
        	echo -en "\e[1;33m[+] Scan finished!\e[0m 🙂\n"
            cat $output_file
        fi
        exit 0
        ...
        3) # masscan
        echo -en "\e[0;32m[*] Do you want to run the default scan for masscan? (y/n):~ \e[0m"
        read run_default_scan
        echo -en "\n"
        ...
        4) # Unicornscan 
        echo -en "\e[0;32m[*] Do you want to run the default scan for unicornscan? (y/n):~ \e[0m"
        read run_default_scan
        echo -en "\n"
        ...
        5) # Netcat
        echo -en "\e[0;32m[*] Do you want to run the default scan for netcat? (y/n):~ \e[0m"
        read run_default_scan
        echo -en "\n"
        ...
        6) # Naabu
        echo -en "\e[0;32m[*] Do you want to run the default scan for naabu? (y/n):~ \e[0m"
        read run_default_scan
        echo -en "\n"
        ...
        if [ "$run_default_scan" == "y" ]; then 
            ...
            naabu -p 1-65535 $target_ip --open --rate 10000 -i $interface -oL $output_file
        else 
            read -p "[+] Enter target IP (e.g., '10.10.14.57' or '2603:3001:2d00:da00::'):~ " target_ip
            read -p "[+] Enter port or port range (e.g., 80,1-10000):~ " port
        ...
        output_file="scan_result.rustscan"
        echo -en "\e[1;32m[+] Please wait few seconds to finish the scan...\n\e[0m"
        rustscan --accessible -a $target_ip --top -t 2500 --ulimit 10000 --batch-size 65535 > $output_file
        echo -en "\e[1;33m[+] Scan finished!\e[0m 🙂\n"
        cat $output_file
        echo
        ...
        read -p "[+] Enter interface (e.g., 'tun0', 'eth0'):~ " interface
        output_file="scan_result.masscan"
        sudo masscan -p1-65535 $target_ip --open --rate 1000 -e $interface -oL $output_file
        echo
